    ...
    <div class="max-w-[1500px] mx-auto px-3 sm:px-4">
      <template v-if="allProducts.length">
        <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mb-4 sm:mb-6">
          <h2 class="text-xl sm:text-2xl font-bold text-gray-900">Хиты продаж</h2>
...
const CATEGORIES = [
  { slug: 'televizory', label: 'Телевизоры' },
  { slug: 'stiralnye_mashiny', label: 'Стиральные машины' },
  { slug: 'telefony', label: 'Телефоны' },
  { slug: 'aerogrili', label: 'Аэрогрили' },
]
...
watch(isVisible, async (val) => {
  ...
  const results = await Promise.all(
    CATEGORIES.map(cat =>
      $fetch<any>(`/api/catalog/${cat.slug}/relationships/items/?include=reviews-statistics`)
        .then(res => ({ slug: cat.slug, data: res?.data ?? [], reviews: res?.included?.['reviews-statistics'] ?? [] }))
...
const visibleTabs = computed(() => {
  ...
  for (const cat of CATEGORIES) {
    if (categoryMap.value[cat.slug]?.length) {
      tabs.push({ key: cat.slug, label: cat.label })
    }
  }
  return tabs
})
